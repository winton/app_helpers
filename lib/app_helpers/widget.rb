module AppHelpers
  ...
  private
    ...
    def update_assets(type, index=0)
      ...
      @assets[type] += update_directory(*(to_string(type, index) + [ index, type ]))
  ...
  def widget(*lineage)
    ...
    templates   *(w.assets[:templates].collect { |t| [ File.basename(t), t, w.options ] })
    w.assets[:init].collect do |f|
      @controller.render_to_string(:file => f, :locals => @options)
    end.join("\n")
  end
