...
Define javascript dependencies from outside of the layout.
...
Define CSS dependencies from outside the layout.

The action view:

  - stylesheets 'css_file_1', 'css_file_2', default_stylesheet
...
Output:
  ...
  <link href="/stylesheets/css_file_1.css?xxx" media="screen" rel="stylesheet" type="text/css" />
  <link href="/stylesheets/css_file_2.css?xxx" media="screen" rel="stylesheet" type="text/css" />
  <link href="/stylesheets/controller/action.css?xxx" media="screen" rel="stylesheet" type="text/css" />


=== <tt>templates(*paths, &block)</tt>

Define JST templates from outside the layout. Requires *Template*.
...
=== <tt>rake app_helpers</tt>
  
Installs <tt>app/views/app_helpers</tt>, which includes a few partials required by *Tbl* and *Template*.

=== <tt>rake git:submodule</tt>

Runs <tt>git:submodule:update</tt>, and <tt>git:submodule:pull</tt>.

=== <tt>rake git:submodule:update</tt>

  git submodule init
  git submodule update

=== <tt>rake git:submodule:pull</tt>

Finds git projects in <tt>config</tt>, <tt>public/javascripts</tt>, and <tt>vendor/plugins</tt> and runs
...
<b>What does this call do?</b>

- First run
  - Merges the <tt>options.rb</tt> hashes, with newer versioned widgets taking precedent over older versions
  - Renders javascripts and stylesheets as ERB with options available as local variables
    - Copies them to <tt>widgets/widget_name_widget_name_v2</tt> in their respective public folders

- Every time
  - Adds javascripts, stylesheets, and templates from all versions using the *Asset* helper
  - Merges <tt>options.rb</tt> hashes with options parameter in the widget call
  - Renders <tt>partials/_init.*</tt> to the view (where the widget call was made), making options available as a local variable
  - Renders <tt>javascripts/_init.js.erb</tt> to the javascript *Asset* helper, making options available as a local variable

<b>Versioned widgets?</b>
